...
- Products of many probabilities get **very small** and are hard to work with, so we take logs:
$$
...
- In practice, we often **minimise the negative log-likelihood**:
...
- So the workflow is:
  ...
  3. Multiply by $-1$  
  4. Use `optim()` to **minimise NLL**

---

**Key idea:**
$$
\text{maximise } L(\theta) \;\equiv\; \text{maximise } \ell(\theta) \;\equiv\; \text{minimise } -\ell(\theta)
$$
:::
...
In your groups work through the examples below (both from @sec-glmfit) and discuss the prompts and plots etc. Be prepared to answer questions as I wander around the class.
